...
tehát az összes sort és oszlopot a mátrixban. matrix=[[null,null,null][][]]
...
az oszlopok száma tömbbé alakítva,  [3].fill(null)
ezt töltöd fel null-lal,       [null]
majd végigmész a tömbön,
és meghívsz egy arrow function-t.   matrix => matrix.
...
*/
//const checkValues = (array) => array.map(row => {row.reduce((a,b)=>a==b?b:false)}).indexOf(true) !== -1;
...
const newGame = () => {
    ...
    amely kattintásra meghívja a következő függvényeket:
        ...
        - setMark()
        */
        button.addEventListener('click', () => {initState();addClickListener();
        deleteSigns();setMessage('Playing...');setMark()});  
}
